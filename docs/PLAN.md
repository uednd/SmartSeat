...
7. 所有明确列入风险清单的开放项均已关闭、降级、延期或由负责人签字接受。
...
|---|---|---|---|---|---|
| GOV-01 | 需求追踪矩阵与完成性治理 | 文档/治理 | P0 | 无 | In Progress |
| GOV-02 | ADR 决策包 | 架构/治理 | P0 | GOV-01 | Not Started |
...
| 接口契约 | 无。 |
| 数据变更 | 无。 |
| 测试要求 | 人工检查每个 PRD 大章均有任务覆盖；检查所有 Task ID 均有 Checklist ID。 |
...
| 接口契约 | 无。 |
| 数据变更 | 无。 |
| 测试要求 | ADR 需人工审阅；每个 ADR 有状态、决策、理由、影响范围、回滚方式。 |
...
| 输出 | User、AuthConfig、Seat、Device、Reservation、QRToken、SensorReading、AnomalyEvent、StudyRecord、AdminActionLog 模型；迁移脚本；seed 数据。 |
| 涉及文件/目录 | ORM schema/实体、migration 目录、seed 脚本、`apps/api/src/modules/*` 必要 repository。 |
...
|---|---|---|---|---|
| R01 | ORM 未在当前骨架中落地 | 影响模型、迁移、seed、测试 | GOV-02 必须先决策 | 架构师 |
| R02 | 微信与 OIDC 真实凭据、回调域名、测试环境未明确 | 认证闭环可能阻塞 | API-AUTH-02/03 支持 mock provider；真实联调列为阻塞项 | 产品/运维/架构师 |
| R03 | 毫米波传感器型号未确定 | 固件驱动不可定稿 | FW-01 先做 adapter；型号确定后补具体实现 | 硬件负责人 |
| R04 | 当前无 CI/CD 配置 | Checklist 难以自动化为闸门 | QA-01 可先用本地命令；后续补 CI | 开发负责人 |
| R05 | 部署目标环境未指定 | 只能保证本地/演示部署 | OPS-01 明确“不代表生产”；生产部署另列任务 | 架构师 |
| R06 | 只有 1 个真实终端 | 多座位/排行榜演示依赖 seed 和模拟器 | SIM-01 与 OPS-01 准备多用户、多记录演示数据 | 开发负责人 |
| R07 | Mosquitto 当前允许匿名 | 不满足正式安全要求 | 本地允许；正式设备认证由 ADR 和后续安全任务处理 | 安全/架构师 |
