...
4. 证据可以是代码路径、测试命令输出、OpenAPI 文档、MQTT 日志、截图、录屏、串口日志、E2E 报告、审计日志或人工审查记录。
5. 涉及安全、权限、secret、用户身份、排行榜匿名、管理员操作的任务必须有人工审查结论。
...
- [x] 已建立“项目完成判定规则”，且规则包含 PRD、PLAN、CHECKLIST、证据路径四个维度。
- [x] 已建立 PRD 范围到 Task ID、Checklist ID 的追踪矩阵。
- [x] 所有 P0/P1 任务均有明确目标、非目标、前置条件、输入、输出、涉及目录、验收标准。
...
- [ ] ORM 选型、迁移策略、seed 策略已记录。
- [ ] 测试框架、E2E 框架、测试目录约定已记录。
- [ ] OpenAPI 生成与发布方式已记录。
- [ ] CI/CD 是否本阶段实现已记录；若不实现，已说明本地闸门替代方式。
- [ ] 部署目标范围已记录，且明确本阶段只保证本地/演示环境或其他指定环境。
- [ ] 传感器型号未确定的 adapter 策略已记录。
- [ ] MQTT 设备认证、安全策略、本地匿名 broker 的边界已记录。
- [ ] 微信/OIDC 凭据、回调地址、secret 管理边界已记录。
- [ ] 每个 ADR 均有状态、决策、理由、影响范围、回滚方式。
- [ ] 证据路径已填写：____
- [ ] 审查人：____
- [ ] 审查日期：____
...
| R-GATE-04 | `docs/DEMO.md` 所有演示脚本均通过或已有签字降级说明 | [ ] |  |
