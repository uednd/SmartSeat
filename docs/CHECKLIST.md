...
- [ ] User 模型已包含角色、匿名名、微信 openid/OIDC subject 映射或等价字段。
- [ ] AuthConfig 模型已支持登录模式与必要配置状态。
- [ ] Seat 模型已包含座位编号、业务状态、可用性、不可用原因、维护状态。
- [ ] Device 模型已包含 device_id、绑定座位、在线状态、最后心跳、固件/硬件信息占位。
- [ ] Reservation 模型已支持预约、签到、使用中、取消、no-show、完成、待释放等状态流转所需字段。
- [ ] QRToken 模型已支持 token、过期时间、使用状态、关联预约/座位/设备。
- [ ] SensorReading 模型已支持 presence 四态、raw_value、时间戳、device_id/seat_id。
- [ ] AnomalyEvent 模型已支持异常类型、状态、来源、处理人、处理时间、处理备注。
- [ ] StudyRecord 模型已支持有效学习时长、开始/结束时间、来源预约。
- [ ] AdminActionLog 模型已支持 admin、action、target、reason、detail、timestamp。
- [ ] 索引、唯一约束、外键已按查询和状态机需要配置。
- [ ] migration 可执行，rollback/down 或等价回滚说明已提供。
- [ ] seed 可生成至少 1 个座位、1 个设备、学生/管理员测试账号、排行榜演示数据。
- [ ] 证据路径已填写：____
