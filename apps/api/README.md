...
SmartSeat NestJS 后端服务。当前已具备平台层能力、`API-DB-01` 数据库基线，以及 `API-AUTH-01/02/03` 认证与用户基础能力：Prisma schema、migration、seed、数据库连接健康检查、登录模式配置、用户角色、`/me`、首个管理员引导、微信登录闭环、OIDC 登录闭环和系统 token 签发/解析。预约规则、扫码签到、MQTT 消费、异常检测、统计计算和小程序页面仍未实现。
...
当前认证边界：已实现 STUDENT/ADMIN 区分、首个用户成为管理员、微信/OIDC mock provider 可测闭环和 token 守卫；未实现小程序页面、OIDC 管理员组映射、真实微信外网联调、真实学校 OIDC 外网联调、座位/预约/MQTT 业务能力。
...
编辑 `.env.deploy`，替换数据库密码、MQTT 凭据、认证 token secret、微信/OIDC 配置占位值和当前登录模式。`.env.deploy` 不应提交到 Git。
