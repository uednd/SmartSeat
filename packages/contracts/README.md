...
扫码签到错误码使用稳定的 `ApiErrorCode`：`QR_TOKEN_EXPIRED`、`QR_TOKEN_USED`、
`QR_TOKEN_INVALIDATED`、`CHECKIN_DUPLICATED`、`CHECKIN_WINDOW_CLOSED`、
`CHECKIN_CONTEXT_MISMATCH`、`CHECKIN_DISABLED`、`RESERVATION_CANCELLED`、
`DEVICE_OFFLINE` 和通用的 `FORBIDDEN` / `RESERVATION_NOT_ACTIVE`。
...
动态二维码下发使用 `MqttDisplayPayload.qr_token`，二维码内容由
`device_id`、`seat_id`、`timestamp` 和 `qr_token` 组成；后端仍以持久化
QRToken、预约、座位和设备状态作为唯一校验来源。
